...
public class BigClip implements Clip, LineListener {
	...
	public void setLoopPoints(int start, int end) {
		if (start < 0 || start > audioData.length - 1 || end < 0 || end > audioData.length) {
			throw new IllegalArgumentException("Loop points '" + start + "' and '" + end + "' cannot be set for buffer of size " + audioData.length);
		}
		if (start > end) {
	...
	public void open(AudioInputStream stream) throws IOException, LineUnavailableException {
		...
		if (format.getChannels() < 2) {
			afTemp = new AudioFormat(format.getEncoding(), format.getSampleRate(), format.getSampleSizeInBits(), 2, format.getSampleSizeInBits() * 2 / 8, // calculate
																																							// frame
																																							// size
	...
	public void open(AudioFormat format, byte[] data, int offset, int bufferSize) throws LineUnavailableException {
		...
		// NOTMYJOBTODO - throw IAE for invalid frame size, format.
	}
	...
	/** NOTMYJOBTODO - fix bug in LOOP_CONTINUOUSLY */
	@Override
	public void start() {
		Runnable r = new Runnable() {
			public void run() {
				try {
					 ...
					 * called by user program? The JavaDocs for line suggest
					 * that Clip should throw an IllegalArgumentException, so
					...
					dataLine.stop();
					/*
					 * should these open()/close() be here, or explicitly called
					 * by user program?
					 */
					dataLine.close();
					...
					if (parent != null) {
						JOptionPane.showMessageDialog(parent, "Clear the sound lines to proceed", "No audio lines available!", JOptionPane.ERROR_MESSAGE);
					}
				}
			}
		};
